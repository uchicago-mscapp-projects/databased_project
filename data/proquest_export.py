...
Associated files: proquest_process.py
...
Purpose: Proquest allows users to create a dataset of articles from various 
newspapers. Users can create a dataset by selecting a publication, passing any 
desired search terms, and applying fitler parameters (i.e. date, article type)
This modules takes a Proquest dataset (a folder of xml files - one for each 
newssource sepcified below) from April 11, 2022 (the date of the first mayoral 
candidate's annoucement) to February 28, 2023 (election day) with the keyword 
"mayor". This module converts each file to a JSON, and appends the JSON to a 
list. The final list is converted to a pandas dataframe, then compressed with 
parquet and tarball to stay within the Proquest 15MB weekly download limit. 
